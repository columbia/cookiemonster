...
const fs = require("fs");
const filePath = path.join(__dirname, "QueryReport.txt");

const adtechnew = express()

adtechnew.use(express.json())
adtechnew.use(cookieParser())

adtechnew.set('view engine', 'pug')
adtechnew.set('views', './views/adtechnew')
...
function log(...args) {
  console.log('\x1b[45m%s\x1b[0m', '[from adtechnew server] ', ...args)
}
...
adtechnew.use(function (req, res, next) {
  console.log(
    'Time:',
    Date.now(),
    ' ',
...
adtechnew.get('/ad-click', (req, res) => {
  res.render('ad-click')
})

adtechnew.get('/ad-click-new', (req, res) => {
  res.render('ad-click-new')
})

adtechnew.get('/ad-script-click-element', (req, res) => {
  res.set('Content-Type', 'text/javascript')
  const adClickUrl = `${process.env.ADTECHNEW_URL}/ad-click`
...
adtechnew.get('/ad-script-click-element-new', (req, res) => {
  res.set('Content-Type', 'text/javascript')
  const adClickUrl = `${process.env.ADTECHNEW_URL}/ad-click-new`
  const iframe = `<iframe src='${adClickUrl}' allow='attribution-reporting' width=190 height=190 scrolling=no frameborder=1 padding=0></iframe>`
  res.send(`document.write("${iframe}");`)
})
...
/* -------------------------------------------------------------------------- */
adtechnew.get('/register-source-href', (req, res) => {
    const attributionDestination = process.env.ADVERTISER_URL
    // For demo purposes, sourceEventId is a random ID. In a real system, this ID would be tied to a unique serving-time identifier mapped to any information an adtechnew provider may need
...
adtechnew.get('/register-source-href-new', (req, res) => {
...
adtechnew.get('/seed-source-registration', (req, res) => {
  const attributionDestination = process.env.ADVERTISER_URL
  // For demo purposes, sourceEventId is a random ID. In a real system, this ID would be tied to a unique serving-time identifier mapped to any information an adtechnew provider may need
  const sourceEventId = Math.floor(Math.random() * 1000000000000000)
  epoch = 2
...
adtechnew.get('/conversion', (req, res) => {
...
adtechnew.post(
  ...
  async (req, res) => {
    ...
    fs.appendFile(filePath, data + "\n", (err) => {
      if (err) console.log("Error Writing Results");
      else 
...
adtechnew.post(
  ...
  async (req, res) => {
    // console.log(
    //   '\x1b[1;31m%s\x1b[0m',
    //   `🚀 adtechnew has received one or more verbose debug reports from the browser`
    // )
...
adtechnew.get(
  ...
  async (req, res) => {
    ...
    res.render('source-registration-seeder', {adtechnewUrl,epoch_start,epoch_end})
  }
)

exports.adtechnew = functions.https.onRequest(adtechnew)
