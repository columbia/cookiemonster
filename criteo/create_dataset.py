...
df['click_day'] = df['click_day'] -  df['click_day'].min()

seconds_in_day = 86400
df['day_delay_for_conversion'] = df['Time_delay_for_conversion'].apply(lambda x: x / seconds_in_day)
df['conversion_day'] = df['day_delay_for_conversion'] + df['click_day']
df = df.astype({'conversion_day': 'int32'})
df.loc[df['Sale'] == 0, 'conversion_day'] = -1


df = df[['click_day', 'conversion_day', 'user_id', 'partner_id', 'product_id', 'product_price', 'SalesAmountInEuro']]
df = df.sort_values(by=['click_day', 'conversion_day', 'user_id', 'partner_id', 'product_id'])
df.to_csv("criteo.csv", header=True, index=False)
