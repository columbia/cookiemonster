...
def get_epsilon_from_accuracy_for_counts(
    ...
    return get_epsilon_for_high_probability_absolute_error(sensitivity=cap_value, absolute_error=a * n, failure_probability=b)

def get_epsilon_for_high_probability_relative_error_wrt_prior(
    sensitivity: float, expected_result: float, relative_error: float, failure_probability: float
):
...
def get_epsilon_for_rmse(
    sensitivity: float, rmse: float
):
